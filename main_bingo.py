...
from model import Rnp_model, Fr_model
from optimizers.optimizer_bingo import BINGO

from train_util import train_rnp_bingo
...
if(args.optimizer=="Adam"):
    ...
    optimizer = BINGO(optimizer)
else:
    print("Main: Exception One")
    sys.exit(1)
...
for epoch in range(args.epochs):
    start = time.time()
    model.train()
    train_loss, precision, recall, f1_score, train_accuracy = train_rnp_bingo(model, optimizer, train_loader, device, args,epoch,para)
    end = time.time()
